...
export function CategoriesScreen() {
  ...
  const openCreate = () => { setEditingId(null); setForm(initialForm); setErrors({}); setIsFormOpen(true); };
  ...
  const saveCategory = () => {
    ...
    if (categories.some((c) => c.id !== editingId && c.name.toLowerCase() === form.name.trim().toLowerCase() && c.type === form.type)) nextErrors.name = 'This category already exists.';
  ...
  const deleteCategory = (id: string) => {
    Alert.alert('Delete category?', 'Transactions using it will show as Uncategorized.', [
      ...
      { text: 'Delete', style: 'destructive', onPress: () => setCategories((current) => current.filter((c) => c.id !== id)) },
    ]);
  };
  ...
  return (
    <ScreenShell title="Categories" subtitle="Organize income and expenses" action={<Button label="+ Add category" onPress={openCreate} />}>
      ...
      {isFormOpen ? <ModalCard title={editingId ? 'Edit Category' : 'Add Category'} onClose={() => setIsFormOpen(false)}>
        <Field label="Category name" value={form.name} onChangeText={(name) => setForm((c) => ({ ...c, name }))} placeholder="e.g. Groceries" error={errors.name} />
        <SelectField label="Category type" value={form.type} options={[{ label: 'Income', value: 'income' }, { label: 'Expense', value: 'expense' }]} onChange={(type) => setForm((c) => ({ ...c, type }))} />
        <Field label="Icon" value={form.icon} onChangeText={(icon) => setForm((c) => ({ ...c, icon }))} placeholder="e.g. ◈" />
