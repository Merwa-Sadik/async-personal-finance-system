...
const ExpenseScreen: React.FC = () => {
  ...
  const submit = async () => {
    ...
    if (!description.trim() || !selectedCategory || !Number.isFinite(numericAmount) || numericAmount <= 0) {
      setSaveError('Enter a description, category, and positive amount.')
      return
    }
    setSaving(true)
    setSaveError('')
    try {
      await addTransaction({ type: 'expense', description: description.trim(), category: selectedCategory, amount: numericAmount, date: new Date().toISOString().slice(0, 10) })
      setDescription('')
      setCategory('Food')
      setCustomCategory('')
      setAmount('')
  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={styles.content}>
      <View style={styles.header}><View><Text style={styles.heading}>Expenses</Text><Text style={styles.subtitle}>See where your money is going.</Text></View><TouchableOpacity style={styles.button} onPress={() => setOpen(true)}><Text style={styles.buttonText}>+ Add Expense</Text></TouchableOpacity></View>
      <View style={styles.total}><Text style={styles.label}>Total expenses</Text><Text style={styles.totalValue}>{total.toLocaleString()} ETB</Text><Text style={styles.meta}>{expenses.length} entries</Text></View>
      <View style={styles.card}><Text style={styles.cardTitle}>Expense transactions</Text>{expenses.length === 0 ? <Text style={styles.empty}>No expense entries yet.</Text> : expenses.map((item) => <View style={styles.row} key={item.id}><View style={styles.main}><Text style={styles.rowTitle}>{item.description}</Text><Text style={styles.meta}>{item.category} · {item.date}</Text></View><View style={styles.end}><Text style={styles.amount}>-{item.amount.toLocaleString()} ETB</Text><TouchableOpacity onPress={() => void deleteTransaction(item.id)}><Text style={styles.delete}>Delete</Text></TouchableOpacity></View></View>)}</View>
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}><View style={styles.overlay}><View style={styles.modal}><View style={styles.modalHeader}><Text style={styles.cardTitle}>Add expense</Text><TouchableOpacity onPress={() => setOpen(false)}><Text style={styles.close}>×</Text></TouchableOpacity></View><Field label="Description" value={description} onChangeText={setDescription} placeholder="Groceries or rent" /><CategoryPicker value={category} options={EXPENSE_CATEGORIES} onChange={setCategory} />{category === 'Add category' ? <><Field label="New category" value={customCategory} onChangeText={setCustomCategory} placeholder="Enter a category" /><TouchableOpacity style={styles.categoryButton} onPress={() => customCategory.trim() && setCategory(customCategory.trim())}><Text style={styles.categoryButtonText}>Save category</Text></TouchableOpacity></> : null}<Field label="Amount" value={amount} onChangeText={setAmount} placeholder="0.00" keyboardType="numeric" />{saveError ? <Text style={styles.error}>{saveError}</Text> : null}<TouchableOpacity style={[styles.button, saving && styles.disabledButton]} onPress={() => void submit} disabled={saving}><Text style={styles.buttonText}>{saving ? 'Saving...' : 'Save expense'}</Text></TouchableOpacity></View></View></Modal>
    </ScrollView>
  )
}
